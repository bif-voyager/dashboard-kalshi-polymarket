...
## Коротко для защиты проекта

- данные берутся из **двух Dune saved queries**;
- backend объединяет Kalshi и Polymarket в единый формат;
- фронтенд работает с одним merged endpoint;
- сравнение строится по историческому объему торгов в USD;
- пропуски в данных не маскируются и не заменяются нулями.
